    ...
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                ...
                if kpValue in fiveOrHigherKp:
                    print(kpValue)
                    sound.playChordTone(stream, cMinorHarmonicChordScale[ kp.index(kpValue) ], eNote * len(group))
                else:
                    sound.playChordTone(stream, cMinorHarmonicChordScale[ kp.index(kpValue) ], eNote * len(group), 0.4)
        except KeyboardInterrupt:
            break
