...
scores = cross_val_score(svc, X, y, cv=skf, scoring="f1_macro")
...
scores = cross_val_score(svc, X, y, cv=sskf, scoring="f1_macro")
print(f"Cross validation F1 score w/ StratifiedShuffleSplit: {round(np.mean(scores),3)}")

scores = cross_val_score(svc, X, y, cv=skf, scoring=f1_loose_scorer)
print(f"Cross validation F1 loose score w/ StratifiedKFold: {round(np.mean(scores),3)}")

scores = cross_val_score(svc, X, y, cv=sskf, scoring=f1_loose_scorer)
